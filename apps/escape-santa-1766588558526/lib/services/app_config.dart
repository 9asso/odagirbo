...
class AppConfig {
  ...
  Future<void> _loadConfig() async {
    ...
    _config = json.decode(configString);
        ...
        // 1) Apply cached runtime overrides immediately (fast, offline-safe)
        await _applyCachedRuntimeOverrides();

        // 2) Best-effort fetch of remote overrides (short timeout). Never throws.
        await _fetchAndApplyRemoteRuntimeOverrides();
  }

    String get _appIdentifier {
        final raw = _config['app']?['packageName'] ??
                _config['app']?['identifierId'] ??
                _config['app']?['packageIdentifier'] ??
                '';
        return raw.toString().trim();
    }

    String? get _runtimeOverridesBaseUrl {
        final raw = _config['runtimeOverrides']?['baseUrl'];
        final base = (raw ?? '').toString().trim();
        if (base.isEmpty) return null;
        return base.endsWith('/') ? base.substring(0, base.length - 1) : base;
    }

    String get _runtimeOverridesCacheKey {
        final id = _appIdentifier;
        return id.isEmpty ? 'appsgen_runtime_overrides' : 'appsgen_runtime_overrides_$id';
    }

    Uri? _buildRuntimeOverridesUri() {
        final base = _runtimeOverridesBaseUrl;
        final id = _appIdentifier;
        if (base == null || base.isEmpty) return null;
        if (id.isEmpty) return null;
        return Uri.parse('$base/apps/${Uri.encodeComponent(id)}/runtime-overrides.json');
    }

    Future<void> _applyCachedRuntimeOverrides() async {
        try {
            final prefs = await SharedPreferences.getInstance();
            final cached = prefs.getString(_runtimeOverridesCacheKey);
            if (cached == null || cached.trim().isEmpty) return;

            final decoded = json.decode(cached);
            if (decoded is! Map<String, dynamic>) return;

            _applyRuntimeOverridesDocument(decoded);
        } catch (_) {
            // ignore
        }
    }
    ...
    Future<void> _fetchAndApplyRemoteRuntimeOverrides() async {
        ...
        try {
            ...
            final applied = _applyRuntimeOverridesDocument(decoded);
            if (!applied) return;
    ...
    bool _applyRuntimeOverridesDocument(Map<String, dynamic> doc) {
        ...
        final overrides = doc['overrides'];
        if (overrides is! Map) return false;

        final game = overrides['game'];
        if (game is Map) {
            final url = game['url'];
            if (url is String && url.trim().isNotEmpty) {
                _config['game'] = (_config['game'] is Map<String, dynamic>)
                        ? _config['game']
                        : <String, dynamic>{};
                (_config['game'] as Map<String, dynamic>)['url'] = url.trim();
            }
        ...
        if (screens is Map) {
            _config['screens'] = (_config['screens'] is Map<String, dynamic>)
                    ? _config['screens']
                    : <String, dynamic>{};
            final screensConfig = _config['screens'] as Map<String, dynamic>;

            for (final entry in screens.entries) {
                final screenName = entry.key;
                final screenValue = entry.value;
                if (screenName is! String) continue;
                if (screenValue is! Map) continue;
                final enabled = screenValue['enabled'];
                if (enabled is! bool) continue;

                screensConfig[screenName] = (screensConfig[screenName] is Map<String, dynamic>)
                        ? screensConfig[screenName]
                        : <String, dynamic>{};
                (screensConfig[screenName] as Map<String, dynamic>)['enabled'] = enabled;
            }
        }
