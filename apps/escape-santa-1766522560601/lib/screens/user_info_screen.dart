...
class _UserInfoScreenState extends State<UserInfoScreen>
  ...
  void _navigateToNextScreen(String name, String email) async {
    // Save user details to backend API
    try {
      print('DEBUG: Config packageName: ${_config.packageName}');
      print('DEBUG: Config appTitle: ${_config.appTitle}');
      
      final userApiService = await UserApiService.getInstance();
      final success = await userApiService.saveUserDetails(
        gender: widget.gender,
        fullName: name,
        email: email,
        appId: _config.appTitle,
        packageName: _config.packageName,
      );
      
      if (success) {
        print('User details saved to backend successfully');
      } else {
        print('Failed to save user details to backend');
      }
    } catch (e) {
